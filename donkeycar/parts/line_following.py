...
DEFAULTS = dict(
   ...
   # trusts only the one closer to the gyro anchor. See the comment at
   # the call site for why -- one side reading the ADJACENT lane's
   # boundary is otherwise invisible to the per-side range gates.
   ...
   LF_LANE_RAMP_SEC=1.5,   # seconds to ramp the offset in/out once the
                           ...
                           # command a swerve.
   # Convenience toggle for Mission 2 (lane following, as opposed to riding
   # the yellow itself): "left" or "right" picks which lane to drive in.
   # This is the ONE setting you need to flip -- it pins BOTH halves of the
   # field-tuned pairing documented above LF_LANE_OFFSET for you:
   #   "left"  -> LF_LANE_OFFSET=-0.5, LF_CURVE_GAIN=-0.8
   #   "right" -> LF_LANE_OFFSET=+0.5, LF_CURVE_GAIN=-0.2
   # overriding whatever LF_LANE_OFFSET/LF_CURVE_GAIN happen to say elsewhere
   # in myconfig.py -- leave those two set to your center-line values, don't
   # delete or edit them. If you want a ride position other than lane-center
   # or want to hand-tune the curve gain, set LF_LANE_OFFSET/LF_CURVE_GAIN
   # yourself instead and leave LF_LANE_SIDE as None; the two mechanisms
   # aren't meant to be mixed.
   # None (default) = unchanged line-following behavior (rides the yellow).
   LF_LANE_SIDE=None,
   # Hard safety margin (fraction of image width) kept between the commanded
   # steering target and a LIVE-DETECTED lane boundary (this frame's own
   # yellow fit and/or same-side white cluster — never a stale or
   # dead-reckoned one). The offset/lane-width geometry above already aims
   # for the lane center, but it trusts a CALIBRATED OR LEARNED width; if
   # that's wrong (mis-measured, or a wider/narrower real section of track)
   # the geometric target can sit closer to a boundary than intended, or
   # even past it, while still reading as normal control error. This is a
   # belt-and-suspenders check on top of that, not a replacement for it: it
   # only engages when a boundary is actually seen fresh THIS frame, and
   # only clamps the target already computed — the PID/steering math is
   # unchanged. See _steer_error.
   LF_LANE_SAFETY_MARGIN_FRAC=0.05,
   # Session 26-07-25_56: at the same wide-plaza crossing this project has
   # been fighting all along, one frame's "genuine" fresh yellow fit put
   # the car 1.5 lane-widths past a boundary -- a physically impossible
   # one-frame teleport (the existing LF_TEMPORAL_JUMP gate compares
   # against the PREVIOUS frame's position and didn't catch this one; this
   # checks against the independent lane-geometry bound instead). A real
   # fix can be genuinely near or a bit past a boundary (that's real
   # drift, see the kp boost below) -- these bounds are deliberately wide,
   # only for catching a fit that's not describing this lane at all.
   LF_LANE_IMPLAUSIBLE_MIN=-1.0,
   LF_LANE_IMPLAUSIBLE_MAX=2.0,
   # Session 26-07-25_56: on a real sustained curve at that same crossing,
   # narrow band-centroid spread held LF_MIN_HEADING_SPREAD_FRAC's guard
   # engaged for the whole ~2s turn, so there was no curve feed-forward --
   # only the (inherently reactive) position term -- and the car drifted
   # to lane_pos=-0.33 (a third of a lane-width across the yellow) before
   # it caught up. Rather than loosen that guard (shared with plain line
   # following, and validated specifically to STOP wild heading swings),
   # give the position term more authority once it's the only thing
   # working AND the car is already close to a boundary: LF_STEER_KP
   # ramps up to LF_LANE_BOUNDARY_KP_BOOST times its normal value as
   # lane_pos closes in on either edge, starting LF_LANE_BOUNDARY_MARGIN
   # lane-widths out. No-op outside lane-following (_lane_pos is None
   # there) and no-op when lane_pos is mid-lane.
...
class LineFollower:
   ...
   def __init__(self, pid=None, cfg=None):
       ...
       self.center_disagree_max_frac = float(_cfg(cfg, 'LF_CENTER_DISAGREE_MAX_FRAC'))
       ...
       lane_side = _cfg(cfg, 'LF_LANE_SIDE')
       if lane_side == 'left':
           self.lane_offset = -0.5
           self.curve_gain = -0.8   # field-tuned pairing, see LF_LANE_SIDE
       elif lane_side == 'right':
           self.lane_offset = 0.5
           self.curve_gain = -0.2   # field-tuned pairing, see LF_LANE_SIDE
       elif lane_side is not None:
           raise ValueError(
               "LF_LANE_SIDE must be 'left', 'right', or None, got %r"
               % (lane_side,))
       self.lane_side = lane_side
       self.lane_safety_margin_frac = float(
           _cfg(cfg, 'LF_LANE_SAFETY_MARGIN_FRAC'))
       ...
       self._yellow_dr_x = None # gyro-only dead-reckoned yellow x. Unlike
                                 # last_x, this is NEVER overwritten by a
                                 # white-guided estimate -- it only moves by
                                 # measured yaw rate. Anchors the white-guide
                                 # sanity gate and the _split_whites side
                                 # reference so a bad estimate can't validate
                                 # the next one against itself (session
                                 # 26-07-24_43: white-guided drifted onto a
                                 # crossing's parking-stripe paint over ~2s
                                 # of small per-frame steps, each one passing
                                 # the gate because it was only ever checked
                                 # against the previous frame's own already-
                                 # drifted last_x).
       ...
       self._last_wr = None      # SELECTED last frame, per side -- lets
                                 # _split_whites prefer continuity with the
                                 # physical line it was already following
                                 # over whichever cluster is nearest the
                                 # (possibly stale) yellow reference this
                                 # instant. Cleared on a full stop.
       self._last_wl_t = None    # time.time() each was last genuinely
       self._last_wr_t = None    # RE-DETECTED (not just reused) -- lets
                                 # _split_whites expire a side that's gone
                                 # undetected for LF_WHITE_STALE_SEC instead
                                 # of reusing it forever (see LF_WHITE_STALE_SEC).
       ...
       self.lane_pos = None     # Mission 2: last lane_pos (0=yellow,
                                # 1=outer white); None when not lane-following
       ...
       logger.info(
           ...
           "lane_side=%s lane_offset=%+.2f lane_width=%s target_x=%s "
           "roi_top=%.2f proc_width=%s (values come from myconfig.py when "
           "set there — DEFAULTS edits do not apply if myconfig defines "
           "the key)",
           self.kp, self.kd, self.ki, self.kh, self.lane_side,
           self.lane_offset, self.lane_width_px, self.target_x,
           self.roi_top, self.proc_width)
   ...
   def _detect_white(self, Lc, Ac, Bc_abs, bgB):
       ...
       # Same ground-plane shape filter already used for the yellow mask
       # (see _filter_by_shape): a real lane line is on the ground and
       # shrinks toward the horizon, so it's small relative to its distance
       # from the ROI top; a background object clipped by the ROI's top
       # edge (a lit doorway/building edge, a railing) is tall for its
       # distance instead. Local contrast alone doesn't know the
       # difference -- a bright edge against a dark background passes it
       # just as well as a real white line does, which is how the car
       # started steering toward building lights instead of the track.
       white = self._filter_by_shape(white)
   ...
   def detect(self, rgb_img):
       ...
       # Report a neutral heading rather than propagate a fit this
   ...
   def _split_whites(self, white_pts, yellow_fit, w, roi_h, now):
       ...
       for cx, cy, n in white_pts:
           if yellow_fit is not None:
               a, b = yellow_fit
               ref = a * cy + b
           elif self._yellow_dr_x is not None:
               ref = self._yellow_dr_x
   ...
   def _estimate_from_whites(self, wl, wr, w):
       ...
       if self._lane_applied <= -0.25:
           ...
           # Trust whichever is closer to the gyro-anchored expectation
           # instead of splitting the difference.
           anchor = self._yellow_dr_x if self._yellow_dr_x is not None \
               else self.last_x
   ...
   def _lane_pos(self, x_line, w):
       ...
       side (LF_LANE_SIDE=left/right) is being driven, since the sign is
       normalized to the active side. Mission 2 (lane following): this is
       the one place "has the car left its lane" is actually checked,
       reusing the same calibrated/learned yellow-to-white distance the
       offset geometry already uses (LF_LANE_WIDTH_PX / dist_left /
   ...
   def run(self, cam_img):
       ...
       if not found and self._yellow_dr_x is not None and self.gyro_gain != 0.0:
           f_px = (w / 2.0) / np.tan(np.radians(self.cam_hfov) / 2.0)
           self._yellow_dr_x = float(np.clip(
               self._yellow_dr_x - f_px * self.yaw_rate_f * dt, -0.5 * w, 1.5 * w))
       ...
       # Mission 2 boundary gate, extreme case (see LF_LANE_IMPLAUSIBLE_MIN/
       # MAX): a "genuine" fresh yellow fit that implies the car teleported
       # more than a lane-width past a boundary in a single frame isn't
       # describing this lane at all -- reject it like any other bad fix
       # (the temporal-jump gate above already catches most bad fits, but
       # compares against the PREVIOUS frame's position, not lane geometry,
       # and missed one of exactly this kind in session 26-07-25_56).
       if found:
       ...
       if found:
           ...
           self.last_x = x_meas
           self._yellow_dr_x = x_meas
           self.last_heading = heading
           self._update_filters(x_meas, heading, now, dt)
           error, target = self._steer_error(self.x_f, w)
           self.last_err = error
           debug['target'] = target
           # Mission 2 (lane following): where this sits between the two
           # boundaries (0=yellow, 1=outer white; see _lane_pos). This is a
           # real, fresh yellow fit, so it's trusted and DRIVEN ON even if
           # it reports outside [0, 1] -- rejecting a genuine detection
           # here would be worse than acting on it — but it's recorded for
           # visibility (overlay/replay) since a fresh fit reporting a
           # boundary violation is itself worth noticing.
           ...
           # LF_LANE_BOUNDARY_KP_BOOST): ramp the position gain up as the
           # car closes in on either edge of the lane. A no-op mid-lane or
           # outside lane-following (lane_pos is None there).
           kp_eff = self.kp
           ...
           if (self.white_track and abs(self._lane_applied) >= 0.25
               ...
               debug['target'] = target
               debug['x_est'] = virtual
               # Mission 2: recorded for visibility only here, not
               # enforced -- this tracker is the hard-won, heavily-tuned
               # primary blind-bend path (session 38); rejecting its
               # output on a geometry check that's never been validated
               # against real lane-offset footage risks breaking exactly
               # the case it exists for. See the enforced gate below on
               # the more general (and less delicately tuned) estimate
               # chain instead.
           ...
           if x_est is not None and anchor is not None:
               grow = min(1.0 + 2.0 * (now - self.lost_since), 2.5)
               allow = min(self.single_dash_max_jump * w * grow, 0.33 * w)
               if abs(x_est - anchor) > allow:
                   x_est = None
           if x_est is not None:
               # Mission 2 boundary gate (lane following only -- _lane_pos
               # returns None, so this is a no-op, whenever lane-following
               # isn't engaged; it never affects center-line driving). An
               # estimate implying the car is already past the yellow
               # (pos<0) or past the outer white (pos>1), beyond margin,
               # isn't a position inside this lane at all — a white
               # cluster picked from the wrong feature (the general
               # failure family behind this session's recycling-bin
               # (26-07-25_52) and cone (26-07-25_53) incidents, though
               # both of those happened in center-line mode, which this
               # specific gate doesn't cover) produces exactly this kind
               # of geometrically-impossible estimate. Reject it like any
               # other implausible fix instead of steering across/off a
               # boundary the estimate itself says is already behind it.
               pos = self._lane_pos(x_est, w)
               if pos is not None and not (
                       -self.lane_safety_margin_frac
                       <= pos <= 1 + self.lane_safety_margin_frac):
                   x_est = None
           if x_est is not None:
               ...
               debug['target'] = target
               debug['x_est'] = x_est
               self.lane_pos = self._lane_pos(x_est, w)
               debug['lane_pos'] = self.lane_pos
               self.prev_error = None
               self.d_f = 0.0
               ...
               # projections.
               # At a fully pegged lean this now reaches 0, not 0.15:
               # session 26-07-24_48, chairs right-hander -- the sharp-bend
               # floor WAS engaging (hterm pegged at h_clip), but the
               # estimate's position error was reading strongly the other
               # way (-0.7, on the same collapsed-chord projection this
               # comment already says is meaningless at pegged lean) and
               # its leftover 15% weight was enough to drag the floored
               # 0.5 heading command down to a net 0.2-0.4, too weak to
               # complete the corner. If the projection is untrustworthy
               # here, it should contribute nothing, not a fifteenth.
               p_scale = 1.0 - min(abs(hterm) / max(self.h_clip, 1e-6), 1.0)
               steer = (p_scale * self.kp * error + self.i_term
               ...
               if min(now - self.lost_since, now - guided) > self.lost_stop_sec:
                   ...
                   self._last_wl = self._last_wr = None  # stale after a stop
                   self._last_wl_t = self._last_wr_t = None
                   self._yellow_dr_x = None  # stale after a stop
                   self.lane_pos = None
